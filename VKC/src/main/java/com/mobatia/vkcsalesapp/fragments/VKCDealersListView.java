...
import com.mobatia.vkcsalesapp.appdialogs.StateDistrictPlaceDialog.OnDialogItemSelectListener;
...
public class VKCDealersListView extends Fragment implements VKCUrlConstants {
	...
	ImageView imageViewSearch, imageViewSubmit;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (roleId.equals("7")) {
			mRootView = inflater.inflate(
					R.layout.fragment_dealers_listview_subdealer, container,
					false);
		...
		AppController.isDealerList = true;
	...
	private void init(View v, Bundle savedInstanceState) {
		...
		imageViewSubmit.setOnClickListener(new ClickOnView());
	...
	private class ClickOnView implements OnClickListener {
		...
		public void onClick(View v) {
			...
			if (v == imageViewSubmit) {
				...
				for (int i = 0; i < AppController.dealersModels.size(); i++) {
					if (AppController.dealersModels.get(i).isSelected()) {
						AppController.listDealers
								.add(AppController.dealersModels.get(i).getId());
					}
				}
				if (AppController.listDealers.size() != 0) {
					if (AppController.listDealers.size() < 7)
						...
						Toast.makeText(getActivity(),
								"Only six dealers are allowed to add",
								Toast.LENGTH_SHORT).show();

				} else {
					Toast.makeText(getActivity(),
							"Please select dealers before submit",
							Toast.LENGTH_SHORT).show();
				}
			}
		}
						...
						try {
							...
							if (status.equals("Success")) {
								...
								CustomToast toast = new CustomToast(
										getActivity());
								toast.show(31);
								AppPrefenceManager.saveDealerListCount(getActivity(), String.valueOf(AppController.listDealers.size()));
								AppController.listDealers.clear();
								/*
								 * mFragmentManager =
