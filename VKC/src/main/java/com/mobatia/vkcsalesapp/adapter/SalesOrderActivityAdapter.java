...
public class SalesOrderActivityAdapter extends BaseAdapter implements
		VKCDbConstants, VKCUrlConstants {
	...
	private TextView mTxtViewQty, mTxtTotalItem, mtextCartValue;
	private DataBaseManager databaseManager;
	private int cartPrice;

	public SalesOrderActivityAdapter(Activity mActivity,
	...
	public SalesOrderActivityAdapter(Activity cartActivity,
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		holder.qtyWatcher = new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				// Update the quantity
				String text= holder.Edtqty.getText().toString().trim();
				if (text.equals("")
						|| text
								.equals("0")) {
				//	getCartData();
					...
					mAdapter.openToRead();
					mAdapter.updateQuantity(AppController.cartArrayList.get(position).getProdName(),AppController.cartArrayList.get(position).getProdColor(),AppController.cartArrayList.get(position).getProdSize(),holder.Edtqty.getText().toString().trim());
					mAdapter.close();
					AppController.cartArrayList.get(position).setProdQuantity(
							holder.Edtqty.getText().toString());
...
*/
			...
			buttonSet.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					databaseManager = new DataBaseManager(mActivity);
					databaseManager.removeFromDb(TABLE_SHOPPINGCART, "pid",
							AppController.cartArrayList.get(position).getPid());
...
/*
	...
	public void updateCartValue() {
		...
		if (sumValue == null) {
			mtextCartValue.setText("Cart Value: ₹" + 0);

		} else {

			cartPrice = Integer.parseInt(sumValue);
			mtextCartValue.setText("Cart Value: ₹" + sumValue);
		}
		mAdapter.close();
	}
